...
#Statement  path exists (almost surely) from the top-most layer to the bottom-most layer of a 100 ×100 grid graph only if the bond percolation probability exceeds 0.5

def FunToVerifyStatement(NumOFTrials = 50):
    Y_axis = []                                                                     #list to store simulated result of experiment when bond percolation is p                   
    X_axis = [float(i)/float(NumOFTrials) for i in range(NumOFTrials + 1)]           #list containing points from 0 to 1
    for p in X_axis:
        Ts = 0                                                                      #Ts will have the number of resuls when there is path from top to bottom                                                                      #
        Fs = 0                                                                       #Fs will have the number of resuls when there isn't path from top to bottom  
        #simulating experiment 
        for _ in range(NumOFTrials):
            L = Lattice(100)                                                          #creating lattice with bond percolation p                                                    
            L.percolate(p)
            if L.existsTopDownPath():                                                 #checking if there is path from top to bottom 
                Ts += 1                                                                 #incrementing Ts if there is path 
            else:
                Fs += 1         
        print(int(p* NumOFTrials) , '/', NumOFTrials)
        Y_axis.append(Ts/(Ts+Fs))                                                     #appending fraction of positive outputs by total outputs in Y axis list
    #plotting
